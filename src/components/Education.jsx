            ...
            <div className="flex flex-wrap justify-center gap-8">
                {EDUCATION.map((edu, index) => (
                    <div key={index} className="w-full max-w-sm rounded-lg border border-neutral-800 bg-neutral-900 p-6 shadow-lg hover:shadow-xl transition-shadow duration-300">
                        <h3 className="mb-2 text-xl font-bold text-accent">{edu.institution}</h3>
                        <p className="mb-1 text-lg font-semibold text-white">{edu.role}</p>
                        <p className="text-sm text-neutral-400">{edu.duration}</p>
                        <p className="mt-2 font-medium text-green-400">{edu.grade}</p>
                    </div>
                ))}
            </div>
        </div>
